...
if (Cprime(T.get(I_near)) && ~Dprime(T.get(I_near)))
    [x_new, psi_new] = new_state(x, T.get(I_near), 0);
end
if (Dprime(T.get(I_near)) && ~Cprime(T.get(I_near)))
    [x_new, psi_new] = new_state(x, T.get(I_near), 1);
end
if (Dprime(T.get(I_near)) && Cprime(T.get(I_near)))
    pr = rand;
    if (pr < 0.5)
